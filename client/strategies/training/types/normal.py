...
class NormalClient(TrainingStrategy):
    ...
    def fit(
        ...
        if client.selected:
            ...
            # Evaluate on test set immediately after training (for verification)
            ...
            logger.info(
                ...
                + f"Test Acc (immediate): {client.fit_test_acc:.4f}, "
                + f"Test Loss (immediate): {client.fit_test_loss:.4f}"
            )
        else:
