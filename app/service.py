...
app = Flask(__name__)
app.secret_key = "supersecretkey"  # Chave para mensagens flash  # noqa: S105
ROOT = "/home/ozymandias/msc/repositories/fedcia"
generated_compose_file = None


def get_yaml_files(folder: str) -> list[str]:
    yaml_files = []
...
def save_yaml(file_path: str, data: dict) -> None:
    with open(file_path, "w") as file:
        try:
            yaml.safe_dump(
                data, file, default_flow_style=False, allow_unicode=True, explicit_end=False
            )
        except yaml.YAMLError as e:
...
def execute_environment() -> str:
    try:
        # Executa o comando "python3 environment.py" no diretório atual
        result = subprocess.run(  # noqa: S603
            ["/usr/bin/python3", "environment.py"], capture_output=True, text=True, cwd=ROOT
        )
        # Captura o arquivo docker-compose gerado
        generated_compose_file = get_latest_compose_file()  # noqa: F841
...
def get_latest_compose_file() -> str | None:
...
def execute_docker_compose() -> str:
